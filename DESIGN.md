...
1. **Month view** (landing): Apple Calendar-style grid, horizontal swipe between months plus nav-bar chevrons (both ship), today marked, colored dots per day (dots touch — 0px gap), selected-day panel beneath the grid listing that day's entry titles with category icons. Persistent "+" for today.
...
9. **Filter sheet & chips**: a nav-bar filter icon (year and month views) opens a bottom sheet with the two-level category tree — checkmark selection, parents include children, no cap. Active filters become removable chips above the calendar; filtered views show union matches only, with a filtered day's year-view color from its topmost matching Entry. Filters survive navigation, reset on cold launch. A day whose entries all fail the filter renders as an empty cell, never as an error.
